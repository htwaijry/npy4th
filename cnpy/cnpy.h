...
//modified for npy4th, Hani Altwaijry
...
namespace cnpy {
    ...
    struct NpyArray {
        NpyArray(const std::vector<size_t>& _shape, size_t _word_size, char _arrayType, bool _fortran_order) :
            shape(_shape), word_size(_word_size), arrayType(_arrayType), fortran_order(_fortran_order)
        {
            num_vals = 1;
        ...
        NpyArray() : shape(0), word_size(0), arrayType(0), fortran_order(0), num_vals(0) { }
    ...
    void parse_npy_header(FILE* fp,size_t& word_size, char& arrayType, std::vector<size_t>& shape, bool& fortran_order);
    ...
    template<typename T> bool npy_save(std::string fname, const T* data, const std::vector<size_t> shape, std::string mode = "w") {
        FILE* fp = NULL;
	...
	if (fp){
		if(appendMode) {
		    ...
		    bool fortran_order;
		    parse_npy_header(fp,word_size, arrayType, tmp_shape,fortran_order);
    ...
    template<typename T> bool npy_save(std::string fname, const std::vector<T> data, std::string mode = "w") {
        std::vector<size_t> shape;
        shape.push_back(data.size());
        return npy_save(fname, &data[0], shape, mode);
    }

    template<typename T> bool npz_save(std::string zipname, std::string fname, const std::vector<T> data, std::string mode = "w") {
        std::vector<size_t> shape;
        shape.push_back(data.size());
        return npz_save(zipname, fname, &data[0], shape, mode);
    }
